...
一个面向个人实验室、家庭服务器和开发环境的 Windows 临时远程管理方案。

它由以下部分组成：

- Windows PowerShell HTTP Agent：只监听 Windows 本机回环地址
- Chisel 反向隧道：让没有公网入站条件的 Windows 主机主动连接中转服务器
- Linux 中继服务器 管理端：通过反向端口访问 Windows Agent
- 原生 OpenSSH：用于登录中转服务器、检查隧道、上传脚本和执行运维命令

本项目是“脚本执行型远控”，不是交互式远程桌面，也不是完整的 Windows 管理平台。请只在你拥有或明确获授权的电脑上使用。
...
```text
Windows win-lab-a 的 127.0.0.1:18888 → 中继服务器的 127.0.0.1:19101
Windows win-lab-b 的 127.0.0.1:18888 → 中继服务器的 127.0.0.1:19102
```

中继服务器地址、端口、Token、Chisel 认证信息都属于部署者自己的配置，本仓库不提供任何真实值。
...
```bash
git clone https://github.com/<你的用户名>/agent-win-remote.git
cd agent-win-remote
```
...
Token 不是权限隔离的替代品。请把 中继服务器 防火墙、SSH 密钥、Chisel 认证、Agent Token 和操作系统权限一起配置好。
